...
<body>
<form class="form-inline definewidth m20" action="order_hasFinishedOrder.do" method="post">  
 交易时间：
...
<table class="table table-bordered table-hover definewidth m10" align="center">
    ...
    <tr>
     ...
     <c:forEach items="${pagers.datas}" var="order" varStatus="l">
      <tr>
            <td>${order.code}</td>
            <td><fmt:formatDate value="${order.finishTime}" pattern="yyyy-MM-dd"/></td>
            <td>￥${order.totalPrice}</td>
        </tr>
     </c:forEach>
 </table>
 <div class="panel-foot text-center">
      <pg:pager  url="${ctx}/order_hasFinishedOrder.do" maxIndexPages="5" items="${pagers.total}"  maxPageItems="15" export="curPage=pageNumber" >
		<pg:last>  
