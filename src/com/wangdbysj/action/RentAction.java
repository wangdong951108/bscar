...
public class RentAction extends ActionSupport implements ModelDriven<Rent> {
	...
	public String exRentCar() {
		...
		// c.setDailyPrice(dailyPrice);
		// carService.update(c);
		// Car c1 = carService.getById(carId);
		...
		// 根据日期计算价钱
		Long price = (Math.abs(rent.getReturnTime().getTime()
				- rent.getRentTime().getTime()) / (1000 * 60 * 60 * 24))
				* Integer.parseInt(rent.getCar().getDailyPrice());
		if (price != 0) {
